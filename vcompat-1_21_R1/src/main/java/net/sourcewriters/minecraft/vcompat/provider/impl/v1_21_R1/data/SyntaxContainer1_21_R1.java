...
public final class SyntaxContainer1_21_R1 extends WrappedContainer implements IDataContainer {

    private final PersistentDataContainer container;

    public SyntaxContainer1_21_R1(PersistentDataContainer container) {
    ...
    */
    ...
    public boolean has(IKey key, IDataType<?, ?> type) {
        return has(new SyntaxKey(key), WrappedType1_21_R1.wrap(type));
    }
    ...
    public <C> C get(IKey key, IDataType<?, C> type) {
        return get(new SyntaxKey(key), WrappedType1_21_R1.wrap(type));
    }
    ...
    public <V, E> void set(String key, E value, IDataType<V, E> type) {
        set(wrappedKey(key), value, WrappedType1_21_R1.wrap(type));
    }

    @Override
    public <V, E> void set(IKey key, E value, IDataType<V, E> type) {
        set(new SyntaxKey(key), value, WrappedType1_21_R1.wrap(type));
    }
    ...
    @Override
    public IKey[] getKeys() {
        return container.getKeys().stream().map(BukkitKey1_21_R1::new).map(WrappedKey::getNamespacedKey).toArray(IKey[]::new);
    }
    ...
    */
    ...
    @Override
    public SyntaxContext1_21_R1 getWrapContext() {
        return new SyntaxContext1_21_R1(container.getAdapterContext());
    }
    ...
    public <P, C> boolean has(WrappedKey<?> key, WrapType<P, C> type) {
        return container.has(BukkitKey1_21_R1.asBukkit(key), new SimpleBukkitType1_21_R1<>(type));
    }
    ...
    public <P, C> C get(WrappedKey<?> key, WrapType<P, C> type) {
        return container.get(BukkitKey1_21_R1.asBukkit(key), new SimpleBukkitType1_21_R1<>(type));
    }
    ...
    public <B> void set(WrappedKey<?> key, B value, WrapType<?, B> type) {
        container.set(BukkitKey1_21_R1.asBukkit(key), new SimpleBukkitType1_21_R1<>(type), value);
    }

    @Override
    public boolean remove(WrappedKey<?> key) {
        container.remove(BukkitKey1_21_R1.asBukkit(key));
