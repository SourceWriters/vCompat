...
import net.sourcewriters.minecraft.vcompat.provider.impl.v1_21_R1.data.BukkitContext1_21_R1;
import net.sourcewriters.minecraft.vcompat.provider.impl.v1_21_R1.data.BukkitType1_21_R1;
...
public class BukkitConversion1_21_R1 extends BukkitConversion<VersionControl1_21_R1> {
    
    private final Frozen registry = ((CraftServer) Bukkit.getServer()).getServer().registryAccess();

    protected BukkitConversion1_21_R1(VersionControl1_21_R1 versionControl) {
    ...
    public org.bukkit.inventory.ItemStack itemFromCompound(NbtCompound compound) {
        return CraftItemStack.asBukkitCopy(ItemStack.parseOptional(registry, toMinecraftCompound(compound)));
    }

    @Override
    public NbtCompound itemToCompound(org.bukkit.inventory.ItemStack itemStack) {
        return fromMinecraftCompound(CraftItemStack.asNMSCopy(itemStack).save(registry, new CompoundTag()));
    }

    @Override
    public WrappedContext<IDataAdapterContext> createContext(IDataAdapterContext context) {
        return new BukkitContext1_21_R1(context);
    }

    @Override
    public <P, C> WrapType<P, C> wrap(IDataType<P, C> dataType) {
        return new BukkitType1_21_R1<>(dataType);
    }
