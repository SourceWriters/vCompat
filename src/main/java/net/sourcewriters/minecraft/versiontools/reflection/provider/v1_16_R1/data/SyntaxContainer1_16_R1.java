...
public final class SyntaxContainer1_16_R1 extends WrappedContainer implements IDataContainer {

    public static final PersistentDataType<Object, Object> OBJECT_TYPE = new SimpleBukkitType1_16_R1<>(ObjectType.INSTANCE);

    private final PersistentDataContainer container;

    public SyntaxContainer1_16_R1(PersistentDataContainer container) {
    ...
    public IDataContainer getAsSyntaxContainer() {
        return new SyntaxContainer1_16_R1(container);
    }
    ...
    public boolean has(IKey key, DataType<?, ?> type) {
        return has(new SyntaxKey(key), WrappedType1_16_R1.wrap(type));
    }
    ...
    public <C> C get(IKey key, DataType<?, C> type) {
        return get(new SyntaxKey(key), WrappedType1_16_R1.wrap(type));
    }
    ...
    public <E, V> void set(String key, E value, DataType<V, E> type) {
        set(wrappedKey(key), value, WrappedType1_16_R1.wrap(type));
    }

    @Override
    public <E, V> void set(IKey key, E value, DataType<V, E> type) {
        set(new SyntaxKey(key), value, WrappedType1_16_R1.wrap(type));
    }
    ...
    @Override
    public IKey[] getKeys() {
        return container.getKeys().stream().map(BukkitKey1_16_R1::new).map(WrappedKey::getNamespacedKey).toArray(IKey[]::new);
    }
    ...
    @Override
    public SyntaxContext1_16_R1 getContext() {
        return new SyntaxContext1_16_R1(container.getAdapterContext());
    }
    ...
    public boolean has(WrappedKey<?> key) {
        return container.has(BukkitKey1_16_R1.asBukkit(key), OBJECT_TYPE);
    }
    ...
    public <P, C> boolean has(WrappedKey<?> key, WrapType<P, C> type) {
        return container.has(BukkitKey1_16_R1.asBukkit(key), new SimpleBukkitType1_16_R1<>(type));
    }

    @Override
    public Object get(WrappedKey<?> key) {
        return container.get(BukkitKey1_16_R1.asBukkit(key), OBJECT_TYPE);
    }
    ...
    public <P, C> C get(WrappedKey<?> key, WrapType<P, C> type) {
        return container.get(BukkitKey1_16_R1.asBukkit(key), new SimpleBukkitType1_16_R1<>(type));
    }
    ...
    public boolean remove(WrappedKey<?> key) {
        ...
        container.remove(BukkitKey1_16_R1.asBukkit(key));
